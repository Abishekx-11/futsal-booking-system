...
if ($selectedCourt != '' && $selectedDate != '') {
    $slotQuery = mysqli_prepare($conn, "
        ...
        (SELECT COUNT(*) FROM bookings 
         ...
         AND bookings.status IN ('Pending', 'Confirmed')) AS is_booked
        FROM time_slots
        ORDER BY time_slots.start_time
    ");
    mysqli_stmt_bind_param($slotQuery, "is", $selectedCourt, $selectedDate);
    ...
    <form method="GET" action="book_court.php">
        ...
        <select name="court_id" required>
            ...
            while ($court = mysqli_fetch_assoc($courtsResult)) {
                ...
                echo "<option value='" . $court['court_id'] . "' $isSelected>" . htmlspecialchars($court['court_name']) . " (Rs. " . htmlspecialchars($court['price_per_hour']) . "/hr)</option>";
            }
            ?>
        </select>

        <label>Date</label>
        <input type="date" name="booking_date" value="<?php echo htmlspecialchars($selectedDate); ?>" min="<?php echo date('Y-m-d'); ?>" required>
...
<?php if (!empty($slots)) { ?>
    ...
    <form method="POST" action="confirm_booking.php">
        <input type="hidden" name="court_id" value="<?php echo htmlspecialchars($selectedCourt); ?>">
